...
const Location = () => {
  ...
  const { setLocationData, dataColombia: dataCol} = useProvider();

  const Schema = yup
    .object({
      departamento: yup.string().required("Elije una opción"),
      municipio: yup.string().required("Elije una opción"),
    })
    .required();
  ...
  const tempSingleMunicipio = tempMunicipios?.find(
    (dep) => dep.MUNICIPIO === municipio
  );
        ...
        <section className="bg-left col-span-1 md:col-span-2 p-4 flex items-center justify-end">
          <article className="title-cont text-left pl-8">
            ...
            <h2 className=" text-1xl md:text-4xl font-normal uppercase">
              Selecciona la <br className="hidden md:block" /> localización
              geográfica
            </h2>
            <Breadcrumb />
          </article>
              ...
              <div className="gap-4 grid grid-cols-1 md:grid-cols-2 mt-3">
                <div className="col-span-1 md:col-span-2">
                  <Select
                    ...
                    errorMessage={
                      formik.errors.departamento && isSubmitting
                  ...
                  >
                    {departamentos.map((dep) => (
                      <SelectItem key={dep} value={dep} textValue={dep}>
                        {dep}
                      </SelectItem>
                    ))}
                  </Select>
                  ...
                  <Select
                    ...
                    onBlur={formik.handleBlur}
                    errorMessage={
                      formik.errors.municipio && isSubmitting
